    ...
    IUnitOfWork unitOfWork) : IBusinessSettingsUseCase
{
    public async Task<Result<BusinessSettingsDto>> GetAsync(Guid tenantId, CancellationToken cancellationToken)
    {
        ...
        if (!HasPrintingModule(enabledPermissions))
        {
            dto = dto with { EnableOrderPrintZones = false };
        }
    ...
    public async Task<Result<BusinessSettingsDto>> UpdateAsync(Guid tenantId, UpdateBusinessSettingsRequest request, CancellationToken cancellationToken)
    {
        ...
        var enableOrderPrintZones = HasPrintingModule(enabledPermissions) && request.EnableOrderPrintZones;
        ...
        var values = new Dictionary<string, (string Value, string Type)>
        {
            ...
            [SettingsDefaults.EnableOrderPrintZones] = (enableOrderPrintZones.ToString().ToLowerInvariant(), "boolean")
        };
    ...
    private static bool HasPrintingModule(IReadOnlyCollection<string>? permissions)
        => permissions is not null && permissions.Any(code => code.StartsWith("printing.", StringComparison.Ordinal));
